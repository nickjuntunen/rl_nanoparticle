...
class DQN:
    ...
    def train(self, batch : namedtuple, counter):
        ...
        action = torch.tensor([transition.action for transition in batch], device=self.device)
        reward = torch.tensor([transition.reward for transition in batch], device=self.device)
        next_state = torch.stack([transition.next_state for transition in batch]).to(self.device)

        # calculate target
        self.q_target.eval()
        with torch.no_grad():
            target = reward + self.gamma * torch.max(self.q_target(next_state), dim=1).values
        self.q.train()
        self.q_target.train()
